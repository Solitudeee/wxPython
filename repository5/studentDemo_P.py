...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, id):
        ...
        self.fileOpen = il.Add(wx.Bitmap('img/fileOpen.png', wx.BITMAP_TYPE_PNG))
        self.fileClose = il.Add(wx.Bitmap('img/fileClose.png', wx.BITMAP_TYPE_PNG))
        self.fileNormal = il.Add(wx.Bitmap('img/fileNormal.png', wx.BITMAP_TYPE_PNG))
        ...
        # 给根节点设置图像
        self.tree.SetItemImage(self.root, self.fileOpen, wx.TreeItemIcon_Expanded)
        self.tree.SetItemImage(self.root, self.fileClose, wx.TreeItemIcon_Normal)
    ...
    def AddTreeNodes(self, parentItem, items):
        for i in range(len(items)):
            if type(items[i]) == str:
                # print(items[i])
                newItem = self.tree.AppendItem(parentItem, text=items[i])
                self.tree.SetItemImage(newItem, self.fileNormal, wx.TreeItemIcon_Normal)
            else:
                newItem = self.tree.AppendItem(parentItem, text=items[i][0])
                self.tree.SetItemImage(newItem, self.fileClose, wx.TreeItemIcon_Normal)
                self.tree.SetItemImage(newItem, self.fileOpen, wx.TreeItemIcon_Expanded)
